...
public class Tokenizer {
	...
	public void setup_stopword_dic()
	{
		stop_word=new HashSet<String>();
		add("a");
	    add("able");
	    add("about");
	    add("above");
	...
	public List<String> tokenize(String sentence) {
		...
		String temp[]=sentence.split("\\W+");
		for(int i=0;i<temp.length;i++)
		{
			temp[i]=Cleaner(temp[i]);
			PorterStemmer stemmer = new PorterStemmer();
			stemmer.setCurrent(temp[i]);
			if(stemmer.stem())
			{
			   temp[i]=stemmer.getCurrent();
			}
			if(temp[i].length()>1)
			{
				lst.add(temp[i]);
